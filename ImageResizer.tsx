...
const ImageResizer: React.FC<ImageResizerProps> = ({ language }) => {
  ...
  const handleRemoveBackground = async () => {
    ...
    try {
        const { dataUrl: newImageDataUrl } = await removeBackground(previewUrl, originalFile.name);
  ...
  const handleEnhanceQuality = async () => {
    ...
    try {
        const { dataUrl: newImageDataUrl } = await enhanceQuality(previewUrl, originalFile.name);
        setResizedUrl(newImageDataUrl);
