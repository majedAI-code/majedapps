...
const ImageResizer: React.FC<ImageResizerProps> = ({ language }) => {
  ...
  const handleResize = useCallback(async () => {
    ...
    try {
      ...
      const out = canvas.toDataURL(originalFile.type || 'image/png');
      setResizedUrl(out);
  ...
  const handleRemoveBackground = async () => {
    if (!previewUrl || !originalFile) return setError(t.errorNoImage);
    setError(null);
              ...
              >
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="mb-2 text-sm text-slate-400">
                    <span className="font-semibold text-cyan-400">{t.uploadImage}</span> {t.orDrop}
                  </p>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 flex-grow flex flex-col">
            ...
            {resizedUrl && (
              <a
                href={resizedUrl}
                download={downloadFilename || 'output.png'}
